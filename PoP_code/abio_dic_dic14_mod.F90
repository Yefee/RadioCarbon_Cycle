...
! !USES:
   ...
   use named_field_mod, only: named_field_get_index, named_field_set, named_field_register, named_field_get
...
!-------------------------------------------------------------------------
   integer (int_kind) :: &
      ...
      atm_d14c_nf_ind     = 0, &       ! atmospheric d14C
      sflux_c14_nf_ind   =0, &       ! seasurface flux for 14CO2
      sflux_co2abio_nf_ind =0        ! seasurface flux for abiotic CO2
...
!                                 exit_on_err=.false.)
      call named_field_get_index('ATM_CO2abio', atm_co2_nf_ind, &
                                 exit_on_err=.false.)   !use the abiotic co2 instead of biaotic 
...
!-----------------------------------------------------------------------
!    Get d14c index from coupler for reading of d14c data later
!    (each timesstep) and abort if it isn't there
!-----------------------------------------------------------------------

      call named_field_get_index('ATM_D14C', atm_d14c_nf_ind, &
                                 exit_on_err=.false.)

      if (atm_d14c_nf_ind == 0) then
         write(message, *) 'abio_dic_dic14_init: abio_dic_dic14 module requires ', &
                           'atmopsheric D14C from the flux coupler and it is not ', &
                           'present'
         call exit_POP(sigAbort, message)
      endif
...
!------------------------------------------------------------------------
!Register Surface flux for C14 and CO2, CO2 is used for potential use
!------------------------------------------------------------------------

      call named_field_register('SFLUX_C14', sflux_c14_nf_ind)
      do iblock=1,nblocks_clinic
         WORK = c0
         call named_field_set(sflux_c14_nf_ind, iblock, WORK)
      end do


      call named_field_register('SFLUX_CO2_abio', sflux_co2abio_nf_ind)
      do iblock=1,nblocks_clinic
         WORK = c0
         call named_field_set(sflux_co2abio_nf_ind, iblock, WORK)
      end do
...
!-----------------------------------------------------------------------
      ...
      case ('coupler')
         ...
         call named_field_get(atm_d14c_nf_ind, iblock, D14C)
...
!-----------------------------------------------------------------------
 ...
 !-----------------------------------------------------------------------
      ...
      !------------------------------------------------------------------------
      ! send surface CO2 and C14 gas flux to coupler, converting units from
      !  nmol/cm^2/s (positive down) to kg CO2/m^2/s (positive down)
      !------------------------------------------------------------------------    
      call named_field_set(sflux_co2abio_nf_ind, iblock, 44.0e-8_r8 * ABIO_DIC_SFLUX_TAVG(:,:,8,iblock))
      call named_field_set(sflux_c14_nf_ind, iblock, 46.0e-8_r8 * ABIO_DIC_SFLUX_TAVG(:,:,9,iblock))
      
!      write(stdout,*) 'co2abio flux is',44.0e-8_r8 * ABIO_DIC_SFLUX_TAVG(:,:,8,iblock)
!      write(stdout,*) 'c14o2 flux is',46.0e-8_r8 * ABIO_DIC_SFLUX_TAVG(:,:,9,iblock)
...
!         call comp_varying_D14C(bid, data_ind_d14c(bid),D14C)
         call named_field_get(atm_d14c_nf_ind, bid, D14C)
