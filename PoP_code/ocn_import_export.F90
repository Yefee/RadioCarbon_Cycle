...
!    o  d14c -- bottom atm level d14c
!    o  co2abio -- bottom atm level abiotic co2
! 
!-----------------------------------------------------------------------
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------
!
!  unpack atmospheric D14C and abiotic co2
!
!-----------------------------------------------------------------------
   if (index_x2o_Sa_d14c > 0) then
      n = 0
      do iblock = 1, nblocks_clinic
         this_block = get_block(blocks_clinic(iblock),iblock)
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o(index_x2o_Sa_d14c,n)
         enddo
         enddo
      enddo
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating D14C halo')
         return
      endif

      call named_field_set(ATM_D14C_nf_ind, WORK1)
   endif
   
!   write(stdout,*) 'd14c is', WORK1(i,j,1)
   
   if (index_x2o_Sa_co2abio > 0) then
      ...
      do iblock = 1, nblocks_clinic
         ...
         do i=this_block%ib,this_block%ie
            n = n + 1
            WORK1(i,j,iblock) = x2o(index_x2o_Sa_co2abio,n)
         enddo
         enddo
      enddo
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'ocn_import_mct: error updating ABIO CO2 halo')
         return
      endif

      call named_field_set(ATM_CO2abio_nf_ind, WORK1)
   endif
   
!   write(stdout,*) 'abio co2 is', WORK1(i,j,1)
...
!-----------------------------------------------------------------------
!
!     pack abio co2 and C14O2 flux, if requested (kg CO2/m^2/s)
!     units conversion occurs where co2 flux is computed
!
!-----------------------------------------------------------------------

   if (index_o2x_Faoo_fc14_ocn > 0) then
      n = 0
      do iblock = 1, nblocks_clinic
         this_block = get_block(blocks_clinic(iblock),iblock)
         do j=this_block%jb,this_block%je
         do i=this_block%ib,this_block%ie
            n = n + 1
            o2x(index_o2x_Faoo_fc14_ocn,n) = &
               SBUFF_SUM(i,j,iblock,index_o2x_Faoo_fc14_ocn)/tlast_coupled
         enddo
         enddo
      enddo
   endif
   
   if (index_o2x_Faoo_fco2abio_ocn > 0) then
      n = 0
      do iblock = 1, nblocks_clinic
         this_block = get_block(blocks_clinic(iblock),iblock)
         do j=this_block%jb,this_block%je
         do i=this_block%ib,this_block%ie
            n = n + 1
            o2x(index_o2x_Faoo_fco2abio_ocn,n) = &
               SBUFF_SUM(i,j,iblock,index_o2x_Faoo_fco2abio_ocn)/tlast_coupled
         enddo
         enddo
      enddo
   endif
!   write(stdout,*) 'SBUFF_SUM(i,j,iblock,index_o2x_Faoo_fco2abio_ocn) is', SBUFF_SUM(i,j,iblock,index_o2x_Faoo_fco2abio_ocn)/tlast_coupled
!   write(stdout,*) 'SBUFF_SUM(i,j,iblock,index_o2x_Faoo_fc14_ocn) is', SBUFF_SUM(i,j,iblock,index_o2x_Faoo_fc14_ocn)/tlast_coupled
...
!-----------------------------------------------------------------------
!
!  allow for abiotic fco2 and C14O2 field to not be registered on first call
!     because init_forcing is called before init_passive_tracers
!  use weight from previous timestep because flux used here is that
!     computed during the previous timestep
!
!-----------------------------------------------------------------------
   ...
   if (index_o2x_Faoo_fc14_ocn > 0) then
      if (C14_F_nf_ind == 0) then
         call named_field_get_index('SFLUX_C14', C14_F_nf_ind, &
                                    exit_on_err=.not. first)
      endif
   ...
   if (index_o2x_Faoo_fco2abio_ocn > 0) then
      if (CO2abio_F_nf_ind == 0) then
         call named_field_get_index('SFLUX_CO2_abio', CO2abio_F_nf_ind, &
                                    exit_on_err=.not. first)
      endif
...
!-----------------------------------------------------------------------
   ...
   if (index_o2x_Faoo_fc14_ocn > 0 .and. C14_F_nf_ind > 0) then
      call named_field_get(C14_F_nf_ind, iblock, WORK(:,:,iblock))
      SBUFF_SUM(:,:,iblock,index_o2x_Faoo_fc14_ocn) = &
         SBUFF_SUM(:,:,iblock,index_o2x_Faoo_fc14_ocn) + delt_last*WORK(:,:,iblock)
   endif
   if (index_o2x_Faoo_fco2abio_ocn > 0 .and. CO2abio_F_nf_ind > 0) then
      call named_field_get(CO2abio_F_nf_ind, iblock, WORK(:,:,iblock))
      SBUFF_SUM(:,:,iblock,index_o2x_Faoo_fco2abio_ocn) = &
         SBUFF_SUM(:,:,iblock,index_o2x_Faoo_fco2abio_ocn) + delt_last*WORK(:,:,iblock)
   endif
