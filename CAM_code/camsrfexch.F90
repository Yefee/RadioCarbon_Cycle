...
!---------------------------------------------------------------------------
  ...
  type cam_out_t 
     ...
     real(r8) :: d14c(pcols)         ! diagnostic co2
     real(r8) :: co2abio(pcols)      ! abiotic co2
     real(r8) :: psl(pcols)
...
!     real(r8) :: fc14_lnd(pcols)         ! c14 flux from lnd
     real(r8) :: fc14_ocn(pcols)         ! c14 flux from ocn
     real(r8) :: fco2abio_ocn(pcols)       ! abiotic co2 flux from ocn
...
!       cam_in(c)%fc14_lnd (:) = 0._r8
       cam_in(c)%fc14_ocn (:) = 0._r8
       cam_in(c)%fco2abio_ocn (:) = 0._r8
       cam_in(c)%fdms     (:) = 0._r8
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_out(c)%d14c(:)     = 0._r8
       cam_out(c)%co2abio(:)  = 0._r8
       cam_out(c)%psl(:)      = 0._r8
...
!-----------------------------------------------------------------------
   ...
   use c14_cycle,        only: c14_transport, c14_i
  ...
  write (iulog,*) "camout_co2diag is", cam_out%co2diag(1)
  write (iulog,*) "camout_prog bottom co2 is", cam_out%co2prog(1)
  
  if (c14_transport()) then
     do i=1,ncol
       cam_out%d14c(i) = state%q(i,pver,c14_i(3))  ! units in permil
     end do
     do i=1,ncol
       cam_out%co2abio(i) = state%q(i,pver,c14_i(2)) * 1.0e+6_r8 *mwdry/mwco2
!       cam_out%co2abio(i) = state%q(i,pver,c_i(4)) * 1.0e+6_r8 *mwdry/mwco2 ! for debug
     end do
     write (iulog,*) "camout_d14c is", cam_out%d14c(1)
     write (iulog,*) "camout_abio bottom co2 is", cam_out%co2abio(1)
  end if
   !
