module restart_physics
  ...
  use c14_cycle,          only: c14_i, c14_transport
...
!
    ...
    type(var_desc_t) :: trefmxav_desc, trefmnav_desc, flwds_desc, landm_desc, sgh_desc, &
         ...
         pblh_desc,  tpert_desc, qpert_desc, flnt_desc, fsds_desc, fsns_desc, d14c_desc, &
         co2abio_desc
  ...
  CONTAINS
    ...
    if ( .not. adiabatic .and. .not. ideal_phys )then
       ...
       if(c14_transport()) then
          ierr = pio_def_var(File, 'D14C', pio_double, hdimids, d14c_desc)
          ierr = pio_def_var(File, 'CO2abio', pio_double, hdimids, co2abio_desc)
       end if
	 ...
	 ii=0
         ...
         if (c14_transport()) then
            ...
            do i=begchunk,endchunk
               ...
               do j=1,pcols
                  ii=ii+1
                  if(j<=ncol) tmpfield(ii) = cam_out(i)%d14c(j)
               end do
            end do
            call pio_write_darray(File, d14c_desc, iodesc, tmpfield, ierr)
            ii=0
            do i=begchunk,endchunk
               ncol = cam_out(i)%ncol
               do j=1,pcols
                  ii=ii+1
                  if(j<=ncol) tmpfield(ii) = cam_out(i)%co2abio(j)
               end do
            end do
            call pio_write_darray(File, co2abio_desc, iodesc, tmpfield, ierr)
         end if
...
!#######################################################################
    ...
    subroutine read_restart_physics(File, cam_in, cam_out, pbuf2d)
     ...
     if ( .not. adiabatic .and. .not. ideal_phys )then
        ...
        if (c14_transport()) then
           ierr = pio_inq_varid(File, 'D14C', vardesc)
           call pio_read_darray(File, vardesc, iodesc, tmpfield, ierr)
           ii=0
           do c=begchunk,endchunk
              do i=1,pcols
                 ii=ii+1
                 cam_out(c)%d14c(i) = tmpfield(ii)
              end do
           end do

           ierr = pio_inq_varid(File, 'CO2abio', vardesc)
           call pio_read_darray(File, vardesc, iodesc, tmpfield, ierr)
           ii=0
           do c=begchunk,endchunk
              do i=1,pcols
                 ii=ii+1
                 cam_out(c)%co2abio(i) = tmpfield(ii)
              end do
           end do
        end if
