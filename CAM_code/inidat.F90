...
!
    ...
    use c14_cycle   , only: c14_implements_cnst, c14_init_cnst
...
!
	  ...
	  call get_block_gcol_d(iam+1,blksiz,gcid)
          ...
          else if (c14_implements_cnst(cnst_name(m_cnst))) then
           call c14_init_cnst(cnst_name(m_cnst),q3tmp, gcid)
           if(masterproc) write(iulog,*) '          ', cnst_name(m_cnst), ' initialized by "c14_init_cnst"'
          else
